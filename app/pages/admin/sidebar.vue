    ...
    <div v-if="form" class="sidebar-editor-layout">
      <div class="sidebar-editor-main">
        <section class="admin-panel sidebar-config-card">
          ...
          <div class="avatar-editor">
            <div class="avatar-preview"><img v-if="form.avatar" :src="form.avatar" alt="头像预览"><span v-else>{{ authorInitial }}</span></div>
            <div><strong>作者头像</strong><p>建议上传正方形 JPG、PNG 或 WebP，系统会自动优化。</p><div class="avatar-actions"><input ref="avatarInput" type="file" accept="image/jpeg,image/png,image/webp" class="hidden" @change="uploadAvatar"><UButton size="sm" color="neutral" variant="outline" icon="i-lucide-upload" :loading="uploading" @click="avatarInput?.click()">{{ form.avatar ? '更换头像' : '上传头像' }}</UButton><UButton v-if="form.avatar" size="sm" color="error" variant="ghost" icon="i-lucide-trash-2" @click="form.avatar = ''">移除</UButton></div><small v-if="uploadError">头像上传失败，请更换图片后重试。</small></div>
          </div>
      ...
      <aside class="sidebar-preview-panel">
        ...
        <div class="author-preview-card">
          ...
          <div class="preview-avatar"><img v-if="form.avatar" :src="form.avatar" alt=""><span v-else>{{ authorInitial }}</span></div>
...
async function uploadAvatar(event: Event) {
  ...
  uploading.value = true; uploadError.value = false
  try {
    const body = new FormData(); body.append('file', file)
    const result = await $fetch<ApiResult<{ url: string }>>('/api/admin/upload?purpose=image', { method: 'POST', body })
