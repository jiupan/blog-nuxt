<template>
  <section class="post-setting-group">
    <div class="post-setting-group-body">
      <UFormField label="标题">
        <UInput v-model="title" placeholder="请输入文章标题" class="w-full" />
      </UFormField>
      <UFormField label="链接别名">
        <UInput v-model="slug" placeholder="留空时自动生成 8 位链接码" class="w-full" />
      </UFormField>
      <UFormField label="摘要">
        <div class="ai-summary-field">
          <UTextarea v-model="summary" :rows="4" placeholder="简要说明文章内容和读者收获" class="w-full" />
  ...
  <section class="post-setting-group">
    <div class="post-setting-group-body">
      <UFormField label="文章封面">
        <div class="cover-field">
          <UInput v-model="cover" placeholder="/uploads/..." class="w-full" />
          <div class="cover-actions">
            <input ref="coverInputRef" type="file" accept="image/*" class="hidden" @change="$emit('uploadCover', $event)">
            <UButton color="neutral" variant="outline" size="sm" icon="i-lucide-upload" :loading="uploadingCover" @click="coverInputRef?.click()">本地上传</UButton>
            <UButton color="neutral" variant="outline" size="sm" icon="i-lucide-images" @click="$emit('openGallery')">从图库选择</UButton>
            <span v-if="coverUploaded" class="cover-state is-success"><UIcon name="i-lucide-check" class="size-4" />已上传</span>
            <span v-if="coverUploadError" class="cover-state is-error"><UIcon name="i-lucide-alert-circle" class="size-4" />上传失败</span>
          </div>
        </div>
      ...
      <UFormField label="展示策略">
        <label class="pin-setting-card" :class="{ 'is-active': isPinned }">
          ...
          <span class="pin-setting-copy">
            <strong>置顶文章</strong>
            <small>优先显示在首页和文章列表。</small>
          </span>
          <span class="pin-setting-state">{{ isPinned ? '已开启' : '关闭' }}</span>
        </label>
      </UFormField>
      <UFormField label="分类">
      ...
      <UFormField label="标签">
        <div class="taxonomy-field taxonomy-field-stacked">
          <div class="post-tag-picker" :class="{ 'is-empty': !tags.length }">
            <label v-for="item in tags" :key="item.id" class="admin-check-pill" :class="{ 'is-selected': tagIds.includes(item.id) }">
              <input v-model="tagIds" type="checkbox" :value="item.id">
              <UIcon v-if="tagIds.includes(item.id)" name="i-lucide-check" class="size-3" />
              <span>{{ item.name }}</span>
            </label>
            <p v-if="!tags.length" class="text-sm text-slate-500">暂无标签，可在这里直接新增。</p>
          </div>
          <UButton class="taxonomy-inline-action" color="neutral" variant="soft" size="sm" icon="i-lucide-tag" @click="$emit('openTagCreator')">新建标签</UButton>
        </div>
      </UFormField>
    </div>
  </section>
  ...
  <section class="post-setting-group">
    <div class="post-setting-group-body">
      <UFormField label="SEO 标题">
        <div class="ai-summary-field">
          <UInput v-model="seoTitle" placeholder="留空时使用文章标题" class="w-full" />
          <div class="ai-summary-actions">
            <UButton
              color="neutral"
              variant="soft"
      ...
      <UFormField label="SEO 描述">
        <UTextarea v-model="seoDescription" :rows="3" placeholder="建议控制在 150 字以内" class="w-full" />
      </UFormField>
    </div>
  </section>
</template>
...
.cover-state.is-success {
  color: #059669;
}

.cover-state.is-error {
  color: #dc2626;
}
...
.admin-check-pill:hover {
  border-color: #a5b4fc;
  background: #ffffff;
  color: #4338ca;
}

.admin-check-pill.is-selected {
  border-color: #a5b4fc;
  background: #eef2ff;
